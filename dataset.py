...
class RibFracDataset(Dataset):
    ...
    def __init__(self, image_dir, label_dir=None, crop_size=64,
            transforms=None, num_samples=6, train=True):
    ...
    def __getitem__(self, idx):
        ...
        if self.transforms is not None:
            for t in self.transforms:
                if not (isinstance(t, transforms.Flip) or isinstance(t, transforms.Noise)):
                    image_rois = [t(image_roi) for image_roi in image_rois]
                
                elif isinstance(t, transforms.Flip):
                    axis = random.randint(0,2)
                    for i in range(len(image_rois)):
                        image_rois.append(t(image_rois[i], axis))
                    for i in range(len(label_rois)):
                        label_rois.append(t(label_rois[i], axis))
                elif isinstance(t, transforms.Noise):
                    for i in range(len(image_rois)):
                        image_rois.append(t(image_rois[i]))
                        label_rois.append(label_rois[i])
